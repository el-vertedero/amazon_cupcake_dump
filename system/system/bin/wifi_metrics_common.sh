...
log_to_kdm_minerva () {
    ...
    local METADATA4=${default}
    local METADATA5=${default}

    local schema_id="anze/2/02330410"
    ...
    do
        ...
        if [[ "${key_val_pair[0]}" == "operation" ]]; then
            ...
            UNIT=${key_val_pair[1]}
        elif [[ "${key_val_pair[0]}" == "metadata5" ]]; then
            METADATA5=${key_val_pair[1]}
        elif [[ "${key_val_pair[0]}" == "metadata4" ]]; then
            METADATA4=${key_val_pair[1]}
    ...
    if [[ "${METADATA5}" != "${default}" ]]; then
        kdm_logStr+="$\"metadata5\"#\"${METADATA5}\""
    fi
    if [[ "${METADATA4}" != "${default}" ]]; then
        kdm_logStr+="$\"metadata4\"#\"${METADATA4}\""
    fi
